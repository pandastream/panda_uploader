...
a.bindRSCEvent=function(){jQuery(this.xhr).one("readystatechange",PandaUploader.bind(this,"onreadystatechange"))};a.onchange=function(){if(this.fileExtensionIsAllowed())this.triggerEvent("onchange");else{this.query.next("[type=file]").remove();this.createField();PandaUploader.alert("You did not select a video file. Please select a valid file.")}};
a.fileExtensionIsAllowed=function(){var b=false,c=this;jQuery.each(this.options.allowed_extensions,function(f,k){if((new RegExp("\\."+k+"$")).test(c.getFile().fileName))b=true});return b};PandaUploader.BaseStrategy=function(){};
