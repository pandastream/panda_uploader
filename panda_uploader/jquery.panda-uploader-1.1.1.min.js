...
Object.prototype[r]&&r.toLowerCase()!="movie"&&X(q,r,e[r]);i.parentNode.replaceChild(q,i);h=q}}return h}function X(d,e,g){var h=y("param");h.setAttribute("name",e);h.setAttribute("value",g);d.appendChild(h)}function ba(d){var e=u(d);if(e&&e.nodeName=="OBJECT")if(j.ie&&j.win){e.style.display="none";(function(){e.readyState==4?ga(d):setTimeout(arguments.callee,10)})()}else e.parentNode.removeChild(e)}function ga(d){if(d=u(d)){for(var e in d)if(typeof d[e]=="function")d[e]=null;d.parentNode.removeChild(d)}}
function u(d){var e=null;try{e=n.getElementById(d)}catch(g){}return e}function y(d){return n.createElement(d)}function fa(d,e,g){d.attachEvent(e,g);H[H.length]=[d,e,g]}function N(d){var e=j.pv;d=d.split(".");d[0]=parseInt(d[0],10);d[1]=parseInt(d[1],10)||0;d[2]=parseInt(d[2],10)||0;return e[0]>d[0]||e[0]==d[0]&&e[1]>d[1]||e[0]==d[0]&&e[1]==d[1]&&e[2]>=d[2]?true:false}function ca(d,e,g,h){if(!(j.ie&&j.mac)){var i=n.getElementsByTagName("head")[0];if(i){g=g&&typeof g=="string"?g:"screen";if(h)Y=w=null;
if(!w||Y!=g){h=y("style");h.setAttribute("type","text/css");h.setAttribute("media",g);w=i.appendChild(h);if(j.ie&&j.win&&typeof n.styleSheets!=p&&n.styleSheets.length>0)w=n.styleSheets[n.styleSheets.length-1];Y=g}if(j.ie&&j.win)w&&typeof w.addRule==v&&w.addRule(d,e);else w&&typeof n.createTextNode!=p&&w.appendChild(n.createTextNode(d+" {"+e+"}"))}}}function D(d,e){if(da){e=e?"visible":"hidden";if(C&&u(d))u(d).style.visibility=e;else ca("#"+d,"visibility:"+e)}}function ea(d){return/[\"<>.;]/.exec(d)!=
null&&typeof encodeURIComponent!=p?encodeURIComponent(d):d}var p="undefined",v="object",M="application/x-shockwave-flash",aa="SWFObjectExprInst",t=window,n=document,A=navigator,Z=false,L=[k],z=[],S=[],H=[],I,R,W,$,C=false,Q=false,w,Y,da=true,j=function(){var d=typeof n.getElementById!=p&&typeof n.getElementsByTagName!=p&&typeof n.createElement!=p,e=A.userAgent.toLowerCase(),g=A.platform.toLowerCase(),h=g?/win/.test(g):/win/.test(e);g=g?/mac/.test(g):/mac/.test(e);e=/webkit/.test(e)?parseFloat(e.replace(/^.*webkit\/(\d+(\.\d+)?).*$/,
...
function(){if(n.readyState=="complete"){n.detachEvent("onreadystatechange",arguments.callee);b()}});t==top&&function(){if(!C){try{n.documentElement.doScroll("left")}catch(d){setTimeout(arguments.callee,0);return}b()}}()}j.wk&&function(){C||(/loaded|complete/.test(n.readyState)?b():setTimeout(arguments.callee,0))}();f(b)}}})();(function(){j.ie&&j.win&&window.attachEvent("onunload",function(){for(var d=H.length,e=0;e<d;e++)H[e][0].detachEvent(H[e][1],H[e][2]);d=S.length;for(e=0;e<d;e++)ba(S[e]);for(var g in j)j[g]=
...
u(aa);if(d&&I){d.parentNode.replaceChild(I,d);if(R){D(R,true);if(j.ie&&j.win)I.style.display="block"}W&&W($)}Q=false}}}}(),SWFUpload;if(SWFUpload==undefined)SWFUpload=function(b){this.initSWFUpload(b)};
...
a.onerror=function(b){this.notifyError(b)};a.notifyError=function(b){if(!this.errorCalled){this.errorCalled=true;this.triggerEvent("onerror",[b,this.getFile()])}};a.onreadystatechange=function(b){this.triggerEvent("onreadystatechange",arguments);var c=null;try{c=b.target.status}catch(f){this.bindRSCEvent();return}if(c=="200"&&b.target.responseText){this.setValue(PandaUploader.parseJSON(b.target.responseText).id);this.triggerEvent("onsuccess",[b])}else{c!="200"&&this.notifyError(b);this.bindRSCEvent()}};
a.bindRSCEvent=function(){jQuery(this.xhr).one("readystatechange",PandaUploader.bind(this,"onreadystatechange"))};a.onchange=function(){var b=false,c=this;jQuery.each(this.options.allowed_extensions,function(f,k){if((new RegExp("\\."+k+"$")).test(c.getFile().fileName))b=true});if(b)this.triggerEvent("onchange");else{this.query.next("[type=file]").remove();this.createField();PandaUploader.alert("You did not select a video file. Please select a valid file.")}};PandaUploader.BaseStrategy=function(){};
...
c.click(PandaUploader.bind(this.upload_strategy,"onCancel"));return this};b.prototype={start:function(c){this.count=this.realTotal=0;if(this.$p.size()!=0){this.$p.find(".progress-inside").size()==0&&this.$p.append('<div class="progress-inside"></div>');this.progress=this.$p.find(".progress-inside");this.progress.css({height:"100%",backgroundImage:"url("+this.options.uploader_dir+"/progress_fg.gif)"});this.setProgress(c,0,c.size);this.$p.css("display","block");var f=this;this.timer=setInterval(function(){f.animateBarBg()},
20)}},setProgress:function(c,f,k){if(k>this.realTotal)this.realTotal=k;if(this.progress){c=Math.ceil(f*100/this.realTotal);if(c>100)c=100;jQuery(this.progress).css("width",c+"%")}},animateBarBg:function(){this.count++;var c=this.$p.css("background-position");c=c?c.split(" "):0;if(this.count==37){this.count=0;this.$p.css("background-position",c+36+"px 0px")}else this.$p.css("background-position",c-1+"px 0px")},reset:function(){clearInterval(this.timer);jQuery(this.progress).css("width","0%");this.$p.css("display",
"none")}}})();
