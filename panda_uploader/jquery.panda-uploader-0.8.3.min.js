...
a.unescapeFilePostParams=function(b){var d=/[$]([0-9a-f]{4})/i,h={},m;if(b!=undefined){for(var g in b.post)if(b.post.hasOwnProperty(g)){m=g;for(var l;(l=d.exec(m))!==null;)m=m.replace(l[0],String.fromCharCode(parseInt("0x"+l[1],16)));h[m]=b.post[g]}b.post=h}return b};a.testExternalInterface=function(){try{return this.callFlash("TestExternalInterface")}catch(b){return false}};a.flashReady=function(){var b=this.getMovieElement();if(b){this.cleanUp(b);this.queueEvent("swfupload_loaded_handler")}else this.debug("Flash called back ready but the flash movie can't be found.")};
a.cleanUp=function(b){try{if(this.movieElement&&typeof b.CallFunction==="unknown"){this.debug("Removing Flash functions hooks (this should only run in IE and should prevent memory leaks)");for(var d in b)try{if(typeof b[d]==="function")b[d]=null}catch(h){}}}catch(m){}window.__flash__removeCallback=function(g,l){try{if(g)g[l]=null}catch(s){}}};a.fileDialogStart=function(){this.queueEvent("file_dialog_start_handler")};
...
(function(b){var d=["swfupload_loaded_handler","file_queued_handler","file_queue_error_handler","file_dialog_start_handler","file_dialog_complete_handler","upload_start_handler","upload_progress_handler","upload_error_handler","upload_success_handler","upload_complete_handler","queue_complete_handler"],h=[];b.fn.swfupload=function(){var m=b.makeArray(arguments);return this.each(function(){var g;if(m.length==1&&typeof m[0]=="object"){g=b(this).data("__swfu");if(!g){var l=m[0],s=b(this);g=[];b.merge(g,
d);b.merge(g,h);b.each(g,function(O,P){var J=P.replace(/_handler$/,"").replace(/_([a-z])/g,function(F,X){return X.toUpperCase()});l[P]=function(){var F=b.Event(J);s.trigger(F,b.makeArray(arguments));return!F.isDefaultPrevented()}});b(this).data("__swfu",new SWFUpload(l))}}else if(m.length>0&&typeof m[0]=="string"){var G=m.shift();(g=b(this).data("__swfu"))&&g[G]&&g[G].apply(g,m)}})};b.swfupload={additionalHandlers:function(){if(arguments.length===0)return h.slice();else b(arguments).each(function(m,
g){b.merge(h,b.makeArray(g))})},defaultHandlers:function(){return d.slice()},getInstance:function(m){return b(m).data("__swfu")}}})(jQuery);function PandaUploader(){}PandaUploader.supportAjaxUpload=function(){var b=document.createElement("INPUT");b.type="file";return"files"in b};PandaUploader.supportAjaxProgressEvents=function(){return typeof XDomainRequest!="undefined"?true:typeof XMLHttpRequest!="undefined"?"withCredentials"in new XMLHttpRequest:null};
PandaUploader.createXRequestObject=function(){if(typeof XDomainRequest!="undefined")return new XDomainRequest;else if(typeof XMLHttpRequest!="undefined"){var b=new XMLHttpRequest;return"withCredentials"in b?b:null}else return null};PandaUploader.getWidgetClass=function(){return PandaUploader.supportAjaxUpload()&&PandaUploader.supportAjaxUpload?PandaUploader.HTML5Widget:PandaUploader.FlashWidget};PandaUploader.bind=function(b,d){return function(){var h=b[d];if(h)return h.apply(b,arguments)}};
...
PandaUploader.FlashWidget=function(b,d,h,m){PandaUploader.BaseWidget.apply(this,[b,d,h]);m=m===undefined?{}:m;this.query.after('<span id="pandauploader_flashwidget_placeholder"></span>');this.swfupload=b.swfupload(jQuery.extend({upload_url:h.api_url+"/videos.json",file_size_limit:0,file_types:"*.*",file_types_description:"All Files",file_upload_limit:0,flash_url:h.uploader_dir+"/swfupload.swf",button_image_url:h.uploader_dir+"/choose_file_button.png",button_width:87,button_height:27,button_placeholder_id:"pandauploader_flashwidget_placeholder",file_post_name:"file",
debug:false},m))};PandaUploader.FlashWidget.prototype=new PandaUploader.BaseWidget;a=PandaUploader.FlashWidget.prototype;a.constructor=PandaUploader.FlashWidget;
...
(function(){function b(g){this.options=g}function d(g){b.apply(this,[g])}function h(g){b.apply(this,[g])}function m(g){this.options=g;this.$p=jQuery("#"+this.options.upload_progress_id);this.$p.css("display","none");this.count=0}UPLOADING=0;STOP=1;jQuery.fn.checkPandaUploaderOptions=function(g){var l=this.parents("form")[0];if(g===undefined){PandaUploader.alert("There was an error setting up the upload form. (The upload parameters were not specified).");return false}if(this.size()==0){PandaUploader.alert("The jQuery element is empty. Method pandaUploader() cannot be executed");
return false}if(!l){PandaUploader.alert("Could not find a suitable form. Please place the call to pandaUploader() after the form, or to be executed onload().");return false}if($(l).find("[name=submit], #submit").length!=0){PandaUploader.alert('An element of your video upload form is incorrect (most probably the submit button). Neither NAME nor ID can be set to "submit" on any field.');return false}return true};jQuery.fn.pandaUploader=function(g,l,s){l=l===undefined?{}:l;if(!this.checkPandaUploaderOptions(g,
l))return false;l=jQuery.extend({upload_filename_id:null,upload_progress_id:null,api_host:"api.pandastream.com",progress_handler:null,uploader_dir:"/panda_uploader",disable_submit_button:true,strategy:"upload_on_submit",widget:null},l);l.api_url=l.api_url||"http://"+l.api_host+"/v2";if(!l.progress_handler)l.progress_handler=new m(l);if(!l.widget)l.widget=PandaUploader.getWidgetClass();g=new l.widget(this,g,l,s);s=null;switch(l.strategy){case "upload_on_submit":s=d;break;case "upload_on_select":s=
h;break;default:s=l.strategy}strategy=new s(l);strategy.setUploadWidget(g);g.setUploadStrategy(strategy);(l=jQuery("#"+l.upload_cancel_button_id))&&l.click(PandaUploader.bind(strategy,"onCancel"));return this};b.prototype={setUploadWidget:function(g){this.widget=g},disableSubmitButton:function(){$(this.getSubmitButton()).attr("disabled",true)},enableSubmitButton:function(){return $(this.getSubmitButton()).removeAttr("disabled")},getSubmitButton:function(){return $(this.widget.getForm()).find("input[type=submit]")},
onwidgetload:function(){},onchange:function(){},onloadstart:function(){},onprogress:function(g){try{this.options.progress_handler&&this.options.progress_handler.setProgress(this.widget.getFile(),g.loaded,g.total)}catch(l){}},onload:function(){},onerror:function(){},onabort:function(){},onsubmit:function(){}};d.prototype=new b;d.prototype.constructor=d;d.prototype.onchange=function(){var g=$("#"+this.options.upload_filename_id);if(g.size()!=0){g.val(this.widget.getFile().name);this.enableSubmitButton()}};
d.prototype.onwidgetload=function(){$(this.widget.getForm()).submit(PandaUploader.bind(this,"onsubmit"));this.options.disable_submit_button&&this.disableSubmitButton()};d.prototype.onloadstart=function(){this.widget.disable();this.options.progress_handler.reset();this.disableSubmitButton();this.options.progress_handler&&this.options.progress_handler.start(this.widget.getFile())};d.prototype.onreadystatechange=function(g){var l=null;try{l=g.target.status}catch(s){return}if(l=="200"&&g.target.responseText){this.widget.setValue(jQuery.parseJSON(g.target.responseText).id);
this.widget.getForm().submit()}};d.prototype.onerror=function(){$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset()};d.prototype.onabort=function(){this.widget.cancel();this.widget.enable();$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset();this.options.disableSubmitButton&&this.disableSubmitButton()};d.prototype.onsubmit=function(){this.widget.start();return false};h.prototype=new b;h.prototype.constructor=h;h.prototype.onchange=
function(){var g=$("#"+this.options.upload_filename_id);if(g.size()!=0){g.val(this.widget.getFile().name);this.widget.start()}};h.prototype.onloadstart=function(){this.options.progress_handler.reset();this.disableSubmitButton();this.options.progress_handler&&this.options.progress_handler.start(this.widget.getFile())};h.prototype.onabort=function(){this.cancel();this.enable();$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset();this.options.disableSubmitButton&&this.disableSubmitButton()};
h.prototype.onerror=function(){$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset()};h.prototype.onreadystatechange=function(g){var l=null;try{l=g.target.status}catch(s){return}if(l=="200"&&g.target.responseText){this.widget.setValue(jQuery.parseJSON(g.target.responseText).id);this.enableSubmitButton()}};m.prototype={start:function(g){this.count=0;if(this.$p.size()!=0){this.$p.find(".progress-inside").size()==0&&this.$p.append('<div class="progress-inside"></div>');
this.progress=this.$p.find(".progress-inside");this.setProgress(g,0,g.size);this.$p.css("display","block");var l=this;this.timer=setInterval(function(){l.animateBarBg()},20)}},setProgress:function(g,l,s){if(this.progress){g=Math.ceil(l*100/s);$(this.progress).css("width",g+"%")}},animateBarBg:function(){this.count++;var g=this.$p.css("background-position");g=g?g.split(" "):0;if(this.count==37){this.count=0;this.$p.css("background-position",g+36+"px 0px")}else this.$p.css("background-position",g-1+
"px 0px")},reset:function(){clearInterval(this.timer);$(this.progress).css("width","0%");this.$p.css("display","none")}}})();
