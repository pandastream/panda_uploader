...
PandaUploader.createXRequestObject=function(){if(typeof XDomainRequest!="undefined")return new XDomainRequest;else if(typeof XMLHttpRequest!="undefined"){var b=new XMLHttpRequest;return"withCredentials"in b?b:null}else return null};PandaUploader.bind=function(b,d){return function(){var h=b[d];if(h)return h.apply(b,arguments)}};PandaUploader.alert=function(b){return alert(b)};PandaUploader.BaseWidget=function(b,d,h){this.query=b;this.signed_params=d;this.options=h};
...
this.widget.enable();$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset();this.options.disableSubmitButton&&this.disableSubmitButton()};d.prototype.onsubmit=function(){this.widget.start();return false};h.prototype=new b;h.prototype.constructor=h;h.prototype.onchange=function(){var f=$("#"+this.options.upload_filename_id);if(f.size()!=0){f.val(this.widget.getFile().name);this.widget.start()}};h.prototype.onloadstart=function(f,i){this.options.progress_handler.reset();
this.disableSubmitButton();this.options.progress_handler&&this.options.progress_handler.start(i)};h.prototype.onabort=function(){this.cancel();this.enable();$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset();this.options.disableSubmitButton&&this.disableSubmitButton()};h.prototype.onerror=function(){$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset()};h.prototype.onreadystatechange=function(f){if(f.target.status=="200"&&f.target.responseText){this.widget.setValue(jQuery.parseJSON(f.target.responseText).id);
this.enableSubmitButton()}};m.prototype={start:function(f){this.count=0;if(this.$p.size()!=0){this.$p.find(".progress-inside").size()==0&&this.$p.append('<div class="progress-inside"></div>');this.progress=this.$p.find(".progress-inside");this.setProgress(f,0,f.size);this.$p.css("display","block");var i=this;this.timer=setInterval(function(){i.animateBarBg()},20)}},setProgress:function(f,i,r){if(this.progress){f=Math.ceil(i*100/r);$(this.progress).css("width",f+"%")}},animateBarBg:function(){this.count++;
var f=this.$p.css("background-position");f=f?f.split(" "):0;if(this.count==37){this.count=0;this.$p.css("background-position",f+36+"px 0px")}else this.$p.css("background-position",f-1+"px 0px")},reset:function(){clearInterval(this.timer);$(this.progress).css("width","0%");this.$p.css("display","none")}}})();
