...
n){b.merge(j,b.makeArray(n))})},defaultHandlers:function(){return e.slice()},getInstance:function(r){return b(r).data("__swfu")}}})(jQuery);function PandaUploader(){}PandaUploader.supportAjaxUpload=function(){var b=document.createElement("INPUT");b.type="file";return"files"in b};PandaUploader.supportAjaxProgressEvents=function(){return typeof XDomainRequest!="undefined"?true:typeof XMLHttpRequest!="undefined"?"withCredentials"in new XMLHttpRequest:null};
PandaUploader.createXRequestObject=function(){if(typeof XDomainRequest!="undefined")return new XDomainRequest;else if(typeof XMLHttpRequest!="undefined"){var b=new XMLHttpRequest;return"withCredentials"in b?b:null}else return null};
...
b.prototype={disableSubmitButton:function(g){$(this.form).find("input[type=submit]").attr("disabled",g)},onLoad:function(){this.uploader.parents("form").eq(0).submit(n(this,"onSubmit"));this.options.disable_submit_button&&this.disableSubmitButton(true)},onSubmit:function(){},onFileQueued:function(){},onStart:function(){},onProgress:function(){},onSuccess:function(){},onError:function(){},onComplete:function(){},triggerEvent:function(g,h,m){if(g=this.options[g])g.apply(this,h);else m&&m()}};e.prototype=
new b;e.prototype.constructor=e;e.prototype.onFileQueued=function(g,h){this.num_files++;this.num_pending++;g=$("#"+this.options.upload_filename_id);if(g.size()!=0){g.val(h.name);this.disableSubmitButton(false)}};e.prototype.onSubmit=function(){if(this.num_files>0){this.uploader.swfupload("startUpload");return false}};e.prototype.onStart=function(g,h){this.status=UPLOADING;this.uploader.swfupload("setButtonDisabled",true);this.options.progress_handler.reset();this.disableSubmitButton(true);this.options.progress_handler&&
this.options.progress_handler.start(h);this.triggerEvent("start",[g,h])};e.prototype.onCancel=function(g){this.uploader.swfupload("cancelUpload","",false);this.uploader.swfupload("setButtonDisabled",false);$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset();this.num_errors=this.num_pending=this.num_files=0;this.options.disableSubmitButton&&this.disableSubmitButton(true);this.uploader.status==UPLOADING&&this.triggerEvent("cancel",[g]);this.status=STOP};e.prototype.onProgress=
function(g,h,m,u){try{this.options.progress_handler&&this.options.progress_handler.setProgress(h,m,u)}catch(v){}};e.prototype.onSuccess=function(g,h,m){this.uploader.val(eval("("+m+")").id);this.num_pending--;this.triggerEvent("success",[g,h,m])};e.prototype.onError=function(g,h,m,u){$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset();this.num_pending--;this.num_errors++;this.triggerEvent("error",[g,h,u],function(){alert("There was an error uploading the file.\n\nHTTP error code "+
u)})};e.prototype.onComplete=function(g){if(this.num_files>0&&this.num_pending<=0&&this.num_errors<=0)if(this.uploader.val()){this.status=STOP;var h=this;this.triggerEvent("complete",[g],function(){h.form.submit()})}else alert("The video ID was not stored on the form")};j.prototype=new b;j.prototype.constructor=j;j.prototype.onFileQueued=function(){this.uploader.swfupload("startUpload")};j.prototype.onStart=function(g,h){this.options.progress_handler.reset();this.disableSubmitButton(true);this.options.progress_handler&&
this.options.progress_handler.start(h);this.triggerEvent("start",[g,h])};j.prototype.onCancel=function(g){this.uploader.swfupload("cancelUpload","",false);this.uploader.swfupload("setButtonDisabled",false);$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset();this.options.disableSubmitButton&&this.disableSubmitButton(true);this.uploader.status==UPLOADING&&this.triggerEvent("cancel",[g])};j.prototype.onProgress=function(g,h,m,u){try{this.options.progress_handler&&this.options.progress_handler.setProgress(h,
m,u)}catch(v){}};j.prototype.onSuccess=function(g,h,m){this.uploader.val(eval("("+m+")").id);this.num_pending--;this.triggerEvent("success",[g,h,m])};j.prototype.onError=function(g,h,m,u){$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset();this.num_pending--;this.num_errors++;this.triggerEvent("error",[g,h,u],function(){alert("There was an error uploading the file.\n\nHTTP error code "+u)})};j.prototype.onComplete=function(g){if(this.uploader.val()){this.disableSubmitButton(false);
...
m*100);$(this.progress).css("width",g+"%")}},animateBarBg:function(){this.count++;var g=this.$p.css("background-position");g=g?g.split(" "):0;if(this.count==37){this.count=0;this.$p.css("background-position",g+36+"px 0px")}else this.$p.css("background-position",g-1+"px 0px")},reset:function(){clearInterval(this.timer);$(this.progress).css("width","0%");this.$p.css("display","none")}}})();
