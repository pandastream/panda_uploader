...
a.enable=function(){return this.swfupload.swfupload("setButtonDisabled",false)};a.abort=function(){this.swfupload.swfupload("cancelUpload","",false);this.add_filename_field&&jQuery("#"+this.filename_field_id).val("");this.triggerEvent("onabort")};a.setValue=function(b){return this.swfupload.val(b)};a.getValue=function(){return this.swfupload.val()};a.getFilename=function(){return this.file?this.file.name:null};
...
this.xhr.sendAsBinary(b.getAsBinary())};a.abort=function(){$(this.getField()).remove();this.createField();this.xhr.abort()};a.getFile=function(){return jQuery(this.getField()).get(0).files[0]};a.setValue=function(b){return this.query.val(b)};a.disable=function(){jQuery(this.getField()).attr("disabled",true)};a.enable=function(){jQuery(this.getField()).removeAttr("disabled")};a.createField=function(){this.query.after('<input type="file" />');jQuery(this.getField()).change(PandaUploader.bind(this,"onchange"))};
a.getField=function(){return this.query.next().get(0)};a.getFilename=function(){var b=this.getFile();return b?b.name||b.fileName:null};a.onerror=function(b){this.notifyError(b)};a.notifyError=function(b){if(!this.errorCalled){this.errorCalled=true;this.triggerEvent("onerror",[b,this.getFile()])}};
...
a.onchange=function(){if(this.validateFileExtension()&&this.validateFileSize())this.triggerEvent("onchange");else{this.query.next("[type=file]").remove();this.createField()}};a.validateFileExtension=function(){var b=false,c=this;jQuery.each(this.options.allowed_extensions,function(f,k){if((new RegExp("\\."+k+"$","i")).test(c.getFile().fileName))b=true});b||PandaUploader.alert("You did not select a video file. Please select a valid file.");return b};
