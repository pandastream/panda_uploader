...
false);this.query.after('<input type="file" />');jQuery(this.getField()).change(this.boundHandler("onchange"));this.triggerEvent("onwidgetload")};
...
this.xhr.send(b)};a.abort=function(){this.xhr.abort()};a.getFile=function(){return jQuery(this.getField()).get(0).files[0]};a.setValue=function(b){return this.query.val(b)};a.disable=function(){jQuery(this.getField()).attr("disabled",true)};a.enable=function(){jQuery(this.getField()).removeAttr("disabled")};a.getField=function(){return this.query.next().get(0)};a.onerror=function(b){this.notifyError(b)};
a.notifyError=function(){if(!this.errorCalled){this.errorCalled=true;this.triggerEvent("onerror",arguments)}};
a.onreadystatechange=function(b){this.triggerEvent("onreadystatechange",arguments);var c=null;try{c=b.target.status;console.log("ok");console.log(c);console.log(b.target.responseText)}catch(f){console.log("exception");this.bindRSCEvent();return}if(c=="200"&&b.target.responseText){this.setValue(PandaUploader.parseJSON(b.target.responseText).id);this.triggerEvent("onsuccess",[b])}else{c!="200"&&this.notifyError();this.bindRSCEvent()}};
a.bindRSCEvent=function(){jQuery(this.xhr).one("readystatechange",PandaUploader.bind(this,"onreadystatechange"))};PandaUploader.BaseStrategy=function(){};
