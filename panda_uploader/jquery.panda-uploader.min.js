...
(function(){function b(e){this.options=e;this.$p=jQuery("#"+this.options.upload_progress_id);this.$p.css("display","none");this.count=0}var c=0,d=0,g=0;jQuery.fn.pandaUploader=function(e,f,h){function o(){m.closest("form").submit(n)}function u(j,k){c++;d++;j=$("#"+f.upload_filename_id);j.size()!=0&&j.val(k.name)}function n(){i.swfupload("startUpload");return false}function r(j,k){f.progress_handler&&f.progress_handler.start(k)}function l(j,k,p,s){try{f.progress_handler&&f.progress_handler.setProgress(k,
p,s)}catch(x){}}function q(j,k,p){m.val(eval("("+p+")").id);d--}function v(j,k,p,s){alert("There was an error uploading the file.\n\nHTTP error code "+s);d--;g++}function w(){d>0&&c!=g||c==g||(m.val()?t().submit():alert("The video ID was not stored on the form"))}function t(){return m.closest("form").get(0)}var m=this;if(e===undefined){alert("There was an error setting up the upload form. (The upload parameters were not specified).");return false}f=f===undefined?{}:f;h=h===undefined?{}:h;if(f.upload_button_id===
...
this.$p.find(".progress-inside");this.setProgress(e,0,e.size);this.$p.css("display","block");var f=this;setInterval(function(){f.animateBarBg()},20)}},setProgress:function(e,f,h){if(this.progress){e=Math.ceil(f/h*100);$(this.progress).css("width",e+"%")}},animateBarBg:function(){this.count++;var e=parseInt(this.$p.css("background-position").split(" "));if(this.count==37){this.count=0;this.$p.css("background-position",e+36+"px 0px")}else this.$p.css("background-position",e-1+"px 0px")}}})();
