...
a.callFlash=function(b,c){c=c||[];var d=this.getMovieElement(),h,g;try{g=d.CallFunction('<invoke name="'+b+'" returntype="javascript">'+__flash__argumentsToXML(c,0)+"</invoke>");h=eval(g)}catch(f){throw"Call to "+b+" failed";}if(h!=undefined&&typeof h.post==="object")h=this.unescapeFilePostParams(h);return h};a.selectFile=function(){this.callFlash("SelectFile")};a.selectFiles=function(){this.callFlash("SelectFiles")};a.startUpload=function(b){this.callFlash("StartUpload",[b])};
...
a.unescapeFilePostParams=function(b){var c=/[$]([0-9a-f]{4})/i,d={},h;if(b!=undefined){for(var g in b.post)if(b.post.hasOwnProperty(g)){h=g;for(var f;(f=c.exec(h))!==null;)h=h.replace(f[0],String.fromCharCode(parseInt("0x"+f[1],16)));d[h]=b.post[g]}b.post=d}return b};a.testExternalInterface=function(){try{return this.callFlash("TestExternalInterface")}catch(b){return false}};a.flashReady=function(){var b=this.getMovieElement();if(b){this.cleanUp(b);this.queueEvent("swfupload_loaded_handler")}else this.debug("Flash called back ready but the flash movie can't be found.")};
a.cleanUp=function(b){try{if(this.movieElement&&typeof b.CallFunction==="unknown"){this.debug("Removing Flash functions hooks (this should only run in IE and should prevent memory leaks)");for(var c in b)try{if(typeof b[c]==="function")b[c]=null}catch(d){}}}catch(h){}window.__flash__removeCallback=function(g,f){try{if(g)g[f]=null}catch(j){}}};a.fileDialogStart=function(){this.queueEvent("file_dialog_start_handler")};
...
(function(b){var c=["swfupload_loaded_handler","file_queued_handler","file_queue_error_handler","file_dialog_start_handler","file_dialog_complete_handler","upload_start_handler","upload_progress_handler","upload_error_handler","upload_success_handler","upload_complete_handler","queue_complete_handler"],d=[];b.fn.swfupload=function(){var h=b.makeArray(arguments);return this.each(function(){var g;if(h.length==1&&typeof h[0]=="object"){g=b(this).data("__swfu");if(!g){var f=h[0],j=b(this);g=[];b.merge(g,
c);b.merge(g,d);b.each(g,function(v,p){var s=p.replace(/_handler$/,"").replace(/_([a-z])/g,function(o,r){return r.toUpperCase()});f[p]=function(){var o=b.Event(s);j.trigger(o,b.makeArray(arguments));return!o.isDefaultPrevented()}});b(this).data("__swfu",new SWFUpload(f))}}else if(h.length>0&&typeof h[0]=="string"){var l=h.shift();(g=b(this).data("__swfu"))&&g[l]&&g[l].apply(g,h)}})};b.swfupload={additionalHandlers:function(){if(arguments.length===0)return d.slice();else b(arguments).each(function(h,
g){b.merge(d,b.makeArray(g))})},defaultHandlers:function(){return c.slice()},getInstance:function(h){return b(h).data("__swfu")}}})(jQuery);
(function(){function b(g){this.options=g;this.$p=jQuery("#"+this.options.upload_progress_id);this.$p.css("display","none");this.count=0}var c=0,d=0,h=0;jQuery.fn.pandaUploader=function(g,f,j){function l(k){f.disable_submit_button&&n.closest("form").find("input[type=submit]").each(function(){$(this).attr("disabled",k)})}function v(){n.closest("form").submit(s);l(true)}function p(k,m){c++;d++;k=$("#"+f.upload_filename_id);if(k.size()!=0){k.val(m.name);l(false)}}function s(){i.swfupload("startUpload");
return false}function o(k,m){l(true);f.progress_handler&&f.progress_handler.start(m)}function r(){i.swfupload("cancelUpload","",false);$("#"+f.upload_filename_id).val("");f.progress_handler.reset();h=d=c=0;l(true);e.stopPropagation()}function x(k,m,q,t){try{f.progress_handler&&f.progress_handler.setProgress(m,q,t)}catch(B){}}function y(k,m,q){n.val(eval("("+q+")").id);d--}function z(k,m,q,t){alert("There was an error uploading the file.\n\nHTTP error code "+t);d--;h++}function A(){if(c>0&&d<=0&&h<=
0)n.val()?u().submit():alert("The video ID was not stored on the form")}function u(){return n.closest("form").get(0)}var n=this;if(g===undefined){alert("There was an error setting up the upload form. (The upload parameters were not specified).");return false}f=f===undefined?{}:f;j=j===undefined?{}:j;if(f.upload_button_id===undefined){alert("You have to specify the button id");return false}if(n.size()==0){alert("The jQuery element is empty. Method pandaUploader() cannot be executed");return false}if(!u()){alert("Could not find a suitable form. Please place the call to pandaUploader() after the form, or to be executed onload().");
return false}if($(u()).find("[name=submit], #submit").length!=0){alert('An element of your video upload form is incorrect (most probably the submit button). Neither NAME nor ID can be set to "submit" on any field.');return false}var w=jQuery("#"+f.upload_cancel_button_id);w&&w.click(r);f=jQuery.extend({upload_filename_id:null,upload_progress_id:null,api_host:"api.pandastream.com",progress_handler:null,uploader_dir:"/panda_uploader",disable_submit_button:false},f);f.api_url=f.api_url||"http://"+f.api_host+
"/v2";if(!f.progress_handler)f.progress_handler=new b(f);var i=this.swfupload(jQuery.extend({upload_url:f.api_url+"/videos.json",file_size_limit:0,file_types:"*.*",file_types_description:"All Files",file_upload_limit:0,flash_url:f.uploader_dir+"/swfupload.swf",button_image_url:f.uploader_dir+"/choose_file_button.png",button_width:87,button_height:27,button_placeholder_id:f.upload_button_id,post_params:g,file_post_name:"file",debug:false},j));i.bind("swfuploadLoaded",v);i.bind("fileQueued",p);i.bind("uploadStart",
o);i.bind("uploadProgress",x);i.bind("uploadSuccess",y);i.bind("uploadError",z);i.bind("uploadComplete",A);return i};b.prototype={start:function(g){if(this.$p.size()!=0){this.$p.find(".progress-inside").size()==0&&this.$p.append('<div class="progress-inside"></div>');this.progress=this.$p.find(".progress-inside");this.setProgress(g,0,g.size);this.$p.css("display","block");var f=this;setInterval(function(){f.animateBarBg()},20)}},setProgress:function(g,f,j){if(this.progress){g=Math.ceil(f/j*100);$(this.progress).css("width",
g+"%")}},animateBarBg:function(){this.count++;var g=parseInt(this.$p.css("background-position").split(" "));if(this.count==37){this.count=0;this.$p.css("background-position",g+36+"px 0px")}else this.$p.css("background-position",g-1+"px 0px")},reset:function(){$(this.progress).css("width","0%");this.$p.css("display","none")}}})();
