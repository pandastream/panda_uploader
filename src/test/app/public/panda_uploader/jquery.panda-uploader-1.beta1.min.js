...
PandaUploader.createWidget=function(){return PandaUploader.supportHTML5Widget()?new PandaUploader.HTML5Widget:new PandaUploader.FlashWidget};PandaUploader.bind=function(b,c){return function(){var f=b[c];if(f)return f.apply(b,arguments)}};PandaUploader.alert=function(b){return alert(b)};PandaUploader.parseJSON=function(b){return jQuery&&jQuery.parseJSON?jQuery.parseJSON(b):eval("("+b+")")};PandaUploader.BaseWidget=function(){};
...
a.onreadystatechange=function(b){this.triggerEvent("onreadystatechange",arguments);var c=null;try{c=b.target.status}catch(f){return}if(c=="200"&&b.target.responseText){this.setValue(PandaUploader.parseJSON(b.target.responseText).id);this.triggerEvent("onsuccess",[b])}};PandaUploader.BaseStrategy=function(){};
