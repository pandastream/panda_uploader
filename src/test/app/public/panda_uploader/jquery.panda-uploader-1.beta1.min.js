...
a.uploadProgress=function(b,c,f,k){b.loaded=f;b.total=k;this.triggerEvent("onprogress",[b])};a.uploadSuccess=function(b,c,f){this.triggerEvent("onreadystatechange",[{target:{status:"200",responseText:f}}])};a.getFile=function(){return this.file};a.start=function(){return this.swfupload.swfupload("startUpload")};a.disable=function(){return this.swfupload.swfupload("setButtonDisabled",true)};a.enable=function(){return this.swfupload.swfupload("setButtonDisabled",false)};
...
false);this.xhr.addEventListener("readystatechange",this.boundHandler("onreadystatechange"),false);this.query.after('<input type="file" />');$(this.getField()).change(this.boundHandler("onchange"));this.triggerEvent("onwidgetload")};
...
PandaUploader.BaseStrategy.prototype={setUploadWidget:function(b){this.widget=b},init:function(){return true},onwidgetload:function(){},onchange:function(){},onloadstart:function(){},onprogress:function(b){try{this.widget.options.progress_handler&&this.widget.options.progress_handler.setProgress(this.widget.getFile(),b.loaded,b.total)}catch(c){}},onload:function(){},onerror:function(){},onabort:function(){},onsubmit:function(){}};
...
a.init=function(){var b=this.widget.query.parents("form")[0];if(!b){PandaUploader.alert("Could not find a suitable form. Please place the call to pandaUploader() after the form, or to be executed onload().");return false}if($(b).find("[name=submit], #submit").length!=0){PandaUploader.alert('An element of your video upload form is incorrect (most probably the submit button). Neither NAME nor ID can be set to "submit" on any field.');return false}return PandaUploader.BaseStrategy.prototype.init.apply(this,
arguments)};a.disableSubmitButton=function(){$(this.getSubmitButton()).attr("disabled",true)};a.enableSubmitButton=function(){return $(this.getSubmitButton()).removeAttr("disabled")};a.getSubmitButton=function(){return $(this.getForm()).find("input[type=submit]")};a.getForm=function(){return this.widget.query.parents("form").get(0)};a.onchange=function(){this.enableSubmitButton()};
a.onwidgetload=function(){if(this.widget){var b=this.getForm();if(b){$(b).submit(PandaUploader.bind(this,"onsubmit"));this.disable_submit_button&&this.disableSubmitButton()}}};a.onloadstart=function(){this.widget.disable();this.widget.options.progress_handler.reset();this.disableSubmitButton();this.widget.options.progress_handler&&this.widget.options.progress_handler.start(this.widget.getFile())};
...
0,c.size);this.$p.css("display","block");var f=this;this.timer=setInterval(function(){f.animateBarBg()},20)}},setProgress:function(c,f,k){if(this.progress){c=Math.ceil(f*100/k);$(this.progress).css("width",c+"%")}},animateBarBg:function(){this.count++;var c=this.$p.css("background-position");c=c?c.split(" "):0;if(this.count==37){this.count=0;this.$p.css("background-position",c+36+"px 0px")}else this.$p.css("background-position",c-1+"px 0px")},reset:function(){clearInterval(this.timer);$(this.progress).css("width",
