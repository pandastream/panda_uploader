...
false);this.xhr.addEventListener("readystatechange",PandaUploader.bind(this,"onreadystatechange"),false);this.query.after('<input type="file" />');$(this.getField()).change(this.boundHandler("onchange"));this.triggerEvent("onwidgetload")};
...
c);this.xhr.send(b)};a.getFile=function(){return $(this.getField()).get(0).files[0]};a.setValue=function(b){return this.query.val(b)};a.disable=function(){$(this.getField()).attr("disabled",true)};a.enable=function(){$(this.getField()).removeAttr("disabled")};a.getField=function(){return this.query.next().get(0)};
a.onreadystatechange=function(b){var c=null;try{c=b.target.status}catch(f){return}if(c=="200"&&b.target.responseText){this.setValue(jQuery.parseJSON(b.target.responseText).id);this.triggerEvent("onsuccess",[b])}};PandaUploader.BaseStrategy=function(){};
