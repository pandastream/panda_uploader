...
if(f.size()!=0){f.val(this.widget.getFile().name);this.enableSubmitButton()}};d.prototype.onwidgetload=function(){$(this.widget.getForm()).submit(PandaUploader.bind(this,"onsubmit"));this.options.disable_submit_button&&this.disableSubmitButton()};d.prototype.onloadstart=function(){this.widget.disable();this.options.progress_handler.reset();this.disableSubmitButton();this.options.progress_handler&&this.options.progress_handler.start(this.widget.getFile())};d.prototype.onreadystatechange=function(f){if(f.target.status==
"200"&&f.target.responseText){this.widget.setValue(jQuery.parseJSON(f.target.responseText).id);this.widget.getForm().submit()}};d.prototype.onerror=function(){$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset()};d.prototype.onabort=function(){this.widget.cancel();this.widget.enable();$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset();this.options.disableSubmitButton&&this.disableSubmitButton()};d.prototype.onsubmit=function(){this.widget.start();
return false};h.prototype=new b;h.prototype.constructor=h;h.prototype.onchange=function(){var f=$("#"+this.options.upload_filename_id);if(f.size()!=0){f.val(this.widget.getFile().name);this.widget.start()}};h.prototype.onloadstart=function(f,l){this.options.progress_handler.reset();this.disableSubmitButton();this.options.progress_handler&&this.options.progress_handler.start(l)};h.prototype.onabort=function(){this.cancel();this.enable();$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset();
this.options.disableSubmitButton&&this.disableSubmitButton()};h.prototype.onerror=function(){$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset()};h.prototype.onreadystatechange=function(f){if(f.target.status=="200"&&f.target.responseText){this.widget.setValue(jQuery.parseJSON(f.target.responseText).id);this.enableSubmitButton()}};m.prototype={start:function(f){this.count=0;if(this.$p.size()!=0){this.$p.find(".progress-inside").size()==0&&this.$p.append('<div class="progress-inside"></div>');
this.progress=this.$p.find(".progress-inside");this.setProgress(f,0,f.size);this.$p.css("display","block");var l=this;this.timer=setInterval(function(){l.animateBarBg()},20)}},setProgress:function(f,l,s){if(this.progress){f=Math.ceil(l*100/s);$(this.progress).css("width",f+"%")}},animateBarBg:function(){this.count++;var f=this.$p.css("background-position");f=f?f.split(" "):0;if(this.count==37){this.count=0;this.$p.css("background-position",f+36+"px 0px")}else this.$p.css("background-position",f-1+
