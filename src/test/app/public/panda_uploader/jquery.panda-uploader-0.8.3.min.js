...
a.start=function(){var b=this.getFile();this.xhr.open("POST",this.options.api_url+"/videos.json",true);this.xhr.setRequestHeader("Cache-Control","no-cache");this.xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");this.xhr.setRequestHeader("X-File-Name",b.fileName);this.xhr.setRequestHeader("X-File-Size",b.fileSize);this.xhr.setRequestHeader("Content-Type","application/octet-stream");this.xhr.send(b)};a.getFile=function(){return $(this.getField()).get(0).files[0]};a.setValue=function(b){return this.query.val(b)};
...
return false};h.prototype=new b;h.prototype.constructor=h;h.prototype.onchange=function(){var f=$("#"+this.options.upload_filename_id);if(f.size()!=0){f.val(this.widget.getFile().name);this.widget.start()}};h.prototype.onloadstart=function(){this.options.progress_handler.reset();this.disableSubmitButton();this.options.progress_handler&&this.options.progress_handler.start(this.widget.getFile())};h.prototype.onabort=function(){this.cancel();this.enable();$("#"+this.options.upload_filename_id).val("");
this.options.progress_handler.reset();this.options.disableSubmitButton&&this.disableSubmitButton()};h.prototype.onerror=function(){$("#"+this.options.upload_filename_id).val("");this.options.progress_handler.reset()};h.prototype.onreadystatechange=function(f){if(f.target.status=="200"&&f.target.responseText){this.widget.setValue(jQuery.parseJSON(f.target.responseText).id);this.enableSubmitButton()}};m.prototype={start:function(f){this.count=0;if(this.$p.size()!=0){this.$p.find(".progress-inside").size()==
...
f+36+"px 0px")}else this.$p.css("background-position",f-1+"px 0px")},reset:function(){clearInterval(this.timer);$(this.progress).css("width","0%");this.$p.css("display","none")}}})();
